...
| IR 质量度量 | 实测数值 | 工业级意义 |
| :--- | :--- | :--- |
| **Hit@1 准确率** | **100.0%** (12/12) | 意图搜索全部首位命中最佳目标代码块（引入跨语言语义对齐） |
| **Hit@3 准确率** | **100.0%** (12/12) | 100% 召回，自适应 BM25 与语义扩展机制下零脱靶 |
| **MRR (平均倒数排名)** | **1.000** | 衡量排序高质量的标准指标（最高满分） |
| **严格跨文件依赖反查召回** | **100.0%** | 单趟批量精准反查外部类/函数接口定义，杜绝口径水分 |
| **胶囊端到端生成延迟 (p50)** | **7.5 ms** | 涵盖倒排检索 + AST 闭包切片 + 批量依赖反查 + 预算压包全流程 |
| **胶囊端到端生成延迟 (p95)** | **22.0 ms** | 保证终端交互与剪贴板注入极速无感 |
