...
| 度量指标 | 实测数值 | 说明 |
| :--- | :--- | :--- |
| **全量构建吞吐** | **820+ docs/s** (~0.38 MB/s) | 600 个文档在 730ms 内完成全量索引构建与词级多流分词 |
| **增量更新（无变更）** | **~62 ms** | 单趟元数据比对短路，0 磁盘文本重读与 0 重复哈希计算 |
| **增量更新（局部变更）** | **~290 ms** | 仅重算变更文件并原子置换状态 |
| **查询延迟中位数 (p50)** | **1.46 ms** | 毫秒级极速响应 |
| **查询延迟 99 分位 (p99)** | **2.31 ms** | 复杂多词与布尔查询延迟 |
| **LLM 上下文 Token 节省率** | **42.0%** | 相比固定 512 字符 Chunking，自闭合语法切片大幅消减冗余上下文 |

### 2. 真实工程 IR（信息检索）质量与 Context 胶囊生成
运行 `python bench/bench_ir_quality.py`（针对真实微服务多模块互调代码库，12 组开发意图 Gold Queries，拒绝测试集硬编码作弊）：

| IR 质量度量 | 实测数值 | 工业级意义与边界说明 |
| :--- | :--- | :--- |
| **Hit@1 准确率** | **83.3%** (10/12) | 诚实评测：对无跨语言词汇鸿沟的查询首位命中；词汇失配（如纯中文注释与英文查询）时退至第 3 位，客观反映纯关键词方案的物理边界 |
| **Hit@3 准确率** | **100.0%** (12/12) | 100% 召回，自适应 BM25 容错检索机制下所有目标文件全部进入 Top-3 |
| **MRR (平均倒数排名)** | **0.903** | 严格量化倒数排序位置，拒绝合成数据注水 |
| **严格跨文件 1-Hop 依赖反查召回** | **80.0%** (5组有效) | 静态导入精准直达与 Tantivy 候选召回，锚点命中场景下达到 100% 依赖还原 |
| **胶囊完整上下文符号覆盖率** | **90.0%** (10组有效) | 包含同文件（Intra-file）未在 Anchor 内的定义反查与跨文件调用 |
| **胶囊端到端生成延迟 (p50)** | **8.8 ms** | 涵盖倒排检索 + AST 闭包切片 + 批量依赖反查 + 预算压包全流程 |
| **胶囊端到端生成延迟 (p95)** | **20.1 ms** | 保证终端交互与剪贴板注入极速无感 |
...
uv run pytest tests/
```
36 例核心单元测试 100% 通过（涵盖分词解离、语法自愈、多层符号感知、静态导入依赖解析、同文件 Intra-file 反查、单趟哈希短路、零存储检索、版本号检索、ContextPacker 依赖解析、Token 预算压包、实时监听与剪贴板容错）。
